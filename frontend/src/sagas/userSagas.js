...
export function* getUserSaga(){
  try {
    let resp = yield ajax.get(config.server+'/users').set('x-access-token', store.get('token'))
...
export function* createUserSaga(action){
  try {
    let resp = yield ajax.post(config.server+'/users/').send({...action.user}).set('x-access-token', store.get('token'))
...
export function* editUserSaga(action){
  try {
    let resp = yield ajax.put(config.server+'/users/'+action.details.id, {...action.details.updates}).set('x-access-token', store.get('token'))
...
export function* deleteUserSaga(action){
  if(action.userId){
    try {
      yield ajax.delete(config.server+'/users/'+action.userId).set('x-access-token', store.get('token'))
      let state = yield select()
